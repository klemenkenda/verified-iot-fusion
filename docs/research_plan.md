...
**Implementation status against this list.** The registry implements last-known value with a staleness bound, exact lag, trailing count/sum/mean/variance/standard deviation/minimum/maximum, missing-count, staleness, calendar features, unit-checked arithmetic, `coalesce`, forecast selection by lead, and a cross-entity mean. Exact quantiles (`median`, `p25`, `p75`, `iqr`) and a trailing `slope` were added on 2026-09-15, together with `mad` and `time_since_max`/`time_since_min` — a spread around the median, and *when* in the window an extremum fell, neither of which is reachable by composing the others. Categorical equality and membership followed on 2026-09-16 as `equals` and `is_in`, with `mode` and `distinct_count` so that a category can be summarised over a window and not only read at its newest point. **Every operator this section names is now implemented.** Difference and ratio exist as arithmetic but are restricted to within-stream pairs by the search space rather than by the DSL.

The quantiles were not a state-budget question, and the bullet below settles why: the raw window is retained, so an order statistic costs what a mean costs. What had been implemented instead was the set expressible as a *constant-space accumulator* — a narrower criterion than this section states, and never the one it asked for. `slope` is the one addition of that group carrying a parity tolerance rather than exact agreement, and the only one needing a unit rule that was not already in the registry, since a trend is a rate and reporting it in the units of a level is a dimensional error the compiler should not permit.
